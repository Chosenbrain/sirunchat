...
const deleteChatHandler = require("../socketControllers/deleteChatHandler"); // Import deleteChatHandler

const createSocketServer = (server) => {
    const io = socket(server, {
        cors: {
            origin: ["http://localhost:3000", "https://sirunchat.com"],
            methods: ["GET", "POST"],
        },
    });
    ...
    io.on("connection", (socket) => {
        ...
        // Register the delete-chat event
        socket.on("delete-chat", (data) => {
            console.log("Backend received delete-chat event with data:", data); // Debug log
            deleteChatHandler(socket, data); // Call deleteChatHandler with the socket and data
        });
